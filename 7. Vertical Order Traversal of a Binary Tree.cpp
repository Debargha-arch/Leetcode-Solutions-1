...
public:
    vector<vector<int>> verticalTraversal(TreeNode* root) 
	{
        ...
        // we ger minC, maxC, cache ready after all the dfs recursive calls
        for(int c = minC; c < maxC+1; c++)
        {
        	...
        	sort(cache[c].begin(), cache[c].end(), [](pair<int, int> &p1, pair<int, int> &p2){
        		// pair first contains row value and second contains node value
        		return (p1.first < p2.first) || ((p1.first == p2.first) && (p1.second < p2.second));
			});
			vector<int> col;
			for(pair<int, int> &p: cache[c])
			{
				// add node values to a seperate vectir
